/* VORTEXUS 26 — GitHub Pages + Firebase shared media backend — BUILD 2026-09-16-006 */
(() => {
  'use strict';
  ...
  async function renderGallery(){
    ...
    try{
      const snap=await db.collection('media').where('status','==','approved').get({source:'server'});
  ...
  async function compressPhoto(file, targetBytes=9.5*1024*1024){
    if(file.size<=targetBytes) return file;
    const bitmap=await createImageBitmap(file);
    let w=bitmap.width,h=bitmap.height;
    const maxDimension=7000;
    if(Math.max(w,h)>maxDimension){const scale=maxDimension/Math.max(w,h);w=Math.round(w*scale);h=Math.round(h*scale);}
    const canvas=document.createElement('canvas');canvas.width=w;canvas.height=h;
    const ctx=canvas.getContext('2d',{alpha:false});ctx.drawImage(bitmap,0,0,w,h);bitmap.close?.();
    const blobFrom=(quality)=>new Promise((resolve,reject)=>canvas.toBlob(b=>b?resolve(b):reject(new Error('COMPRESSION_FAILED')),'image/jpeg',quality));
    let blob=await blobFrom(.90);
    for(const q of [.82,.75,.68,.60,.52,.45]){if(blob.size<=targetBytes)break;blob=await blobFrom(q);}
    if(blob.size>targetBytes){
      const scale=Math.sqrt(targetBytes/blob.size)*.94;
      const nw=Math.max(1,Math.round(w*scale)),nh=Math.max(1,Math.round(h*scale));
      canvas.width=nw;canvas.height=nh;ctx.drawImage(await createImageBitmap(file),0,0,nw,nh);
      blob=await blobFrom(.70);
    }
    if(blob.size>10*1024*1024) throw new Error('PHOTO_COMPRESSION_LIMIT');
  ...
  async function initSite(){
    ...
    $('#uploadForm')?.addEventListener('submit',async e=>{e.preventDefault();const form=e.currentTarget; const f=input.files?.[0],data=new FormData(form),err=$('#uploadError'),ok=$('#uploadNotice');err.hidden=true;ok.hidden=true;
      ...
      if(isVideo&&f.size>max){err.textContent='Ukuran video terlalu besar. Maksimal 100 MB.';err.hidden=false;return}
      ...
      try{const user=await waitForAuthUser();if(!user)throw new Error('AUTH_REQUIRED');if(!cloudinaryReady())throw new Error('CLOUDINARY_NOT_CONFIGURED');let uploadFile=f;if(!isVideo&&f.size>9.5*1024*1024){ok.textContent='⏳ Foto lebih dari 10 MB, sedang dikompres otomatis...';ok.hidden=false;uploadFile=await compressPhoto(f);}if(!isVideo&&uploadFile.size>10*1024*1024)throw new Error('PHOTO_COMPRESSION_LIMIT');const resourceType=isVideo?'video':'image';const endpoint=`https://api.cloudinary.com/v1_1/${encodeURIComponent(window.VORTEXUS_CLOUDINARY.cloudName)}/${resourceType}/upload`;const fd=new FormData();fd.append('file',uploadFile);fd.append('upload_preset',window.VORTEXUS_CLOUDINARY.uploadPreset);fd.append('folder','vortexus26');const uploadRes=await fetch(endpoint,{method:'POST',body:fd});let uploadJson={};try{uploadJson=await uploadRes.json()}catch(_){}if(!uploadRes.ok||!uploadJson.secure_url)throw new Error(`CLOUDINARY: ${uploadJson.error?.message||`HTTP ${uploadRes.status}`}`);const fileUrl=uploadJson.secure_url;await db.collection('media').add({title:String(data.get('title')||'').trim(),uploader:String(data.get('uploader')||'').trim(),category:data.get('category'),type:data.get('type'),jurusan:String(data.get('jurusan')||session),status:'pending',createdAt:firebase.firestore.FieldValue.serverTimestamp(),fileUrl,cloudinaryPublicId:uploadJson.public_id,resourceType:uploadJson.resource_type,uploaderUid:user.uid});const uploadForm=document.getElementById('uploadForm'); if(uploadForm) uploadForm.reset(); if(js){js.value=session;js.disabled=false;} if(preview) preview.innerHTML=''; if(input) input.value='';ok.textContent='✅ Upload berhasil! Sekarang menunggu persetujuan Admin. Setelah disetujui, foto akan muncul untuk SEMUA orang.';ok.hidden=false}catch(ex){console.error(ex);const code=ex?.code||'';let msg=ex?.message||'Terjadi kesalahan tidak diketahui.';if(code==='permission-denied')msg='Firestore menolak penyimpanan data (permission-denied). Pastikan Firestore Rules sudah di-Publish.';else if(code==='unauthenticated')msg='Sesi Firebase tidak aktif. Silakan login lagi.';else if(ex.message==='AUTH_REQUIRED')msg='Sesi login belum siap. Tunggu sebentar lalu coba lagi, atau login ulang.';else if(ex.message==='CLOUDINARY_NOT_CONFIGURED')msg='Cloudinary belum dikonfigurasi.';else if(ex.message==='PHOTO_COMPRESSION_LIMIT')msg='Foto tetap lebih dari 10 MB setelah kompresi otomatis. Coba foto dengan resolusi lebih rendah.';else if(ex.message==='COMPRESSION_FAILED')msg='Foto gagal dikompres otomatis. Coba format JPG/WEBP.';err.textContent=`Upload gagal: ${msg}`;err.hidden=false}}
    );
    await renderGallery();initFilters();initLightbox();
    // Keep the public gallery synchronized after an admin approval without requiring users to clear cache.
    if(firebaseReady&&db){setInterval(()=>renderGallery().catch(()=>{}),20000);}
  }
  ...
  async function initAdmin(){
    ...
    $('#refreshAdmin')?.addEventListener('click',()=>draw());
    ...
    $('#clearData')?.addEventListener('click',async()=>{if(!confirm('Hapus SEMUA data upload? Tindakan ini tidak dapat dibatalkan.'))return;const snap=await db.collection('media').get({source:'server'});for(const d of snap.docs){await d.ref.delete()}draw()});
