...
(() => {
  'use strict';
  // Legacy IndexedDB script is disabled when the Firebase shared backend is configured.
  // This prevents duplicate upload handlers and old local-only storage from interfering.
  if (window.VORTEXUS_FIREBASE_CONFIG) return;
  ...
  async function initSite(){
    ...
    $('#uploadForm')?.addEventListener('submit',async e=>{
      ...
      try{
        ...
        const uploadForm=document.getElementById('uploadForm'); if(uploadForm) uploadForm.reset(); if(js){js.value=activeSession;js.disabled=true;} if(preview) preview.innerHTML=''; if(input) input.value='';
